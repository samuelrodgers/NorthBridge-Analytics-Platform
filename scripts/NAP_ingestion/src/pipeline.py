...
# ── QUARANTINE LOGIC ──────────────────────────────────────────────────────────
#
# Each rule is a (failure_code, failure_reason, predicate) triple.
#   failure_code   — structured enum-style string; must match analytics.v_quarantine_log
#   failure_reason — human-readable description written to raw.quarantine_event
#   predicate      — callable(row) -> bool, True means the row violates this rule
#
# Adding a new rule: append a tuple here. No other code needs to change.
# Rule order does not affect which rows are quarantined, only the order
...
_QUARANTINE_RULES: list[tuple[str, str, object]] = [
    (
        ...
        lambda row: _is_null(row.get("c_id")),
    ),
    (
        "INVALID_AMOUNT",
        "amount is null or non-positive after normalization",
        lambda row: _is_null(row.get("amount")) or (
            not _is_null(row.get("amount")) and row.get("amount") <= 0
        ),
    ),
    (
        "NULL_TIMESTAMP",
        "tx_timestamp is null after normalization",
        lambda row: _is_null(row.get("tx_timestamp")),
    ),
    (
        "INVALID_BASE_CURRENCY",
        "base_cncy is null or unresolvable after normalization",
        lambda row: _is_null(row.get("base_cncy")),
    ),
]
...
def _split_quarantine(
    ...
    A row is quarantined if it violates ANY rule in _QUARANTINE_RULES.
    Quarantined output has one record per violation — a row that breaks
    two rules produces two quarantine records, linked by tx_id.

    This mirrors the UNION ALL design of analytics.v_quarantine_log so
    ...
    # Track which original row indices have at least one violation
    violated_indices: set[int] = set()
    quarantine_records: list[dict] = []

    for idx, row in df.iterrows():
        row_dict = row.to_dict()
        for failure_code, failure_reason, predicate in _QUARANTINE_RULES:
            try:
                fired = predicate(row_dict)
            except Exception:
                # Defensive: if a predicate errors, treat as a violation
                fired = True
            if fired:
    ...
    if quarantine_records:
        quarantine = pd.DataFrame(quarantine_records).reset_index(drop=True)
    else:
        # Preserve schema even when nothing is quarantined
...
def _collect_stats(original: pd.DataFrame, clean: pd.DataFrame, quarantine: pd.DataFrame) -> dict:
    ...
    # ── Section 2 — quarantine breakdown ─────────────────────────────────
    # quarantine now has one record per violation with a structured
    # failure_code column — count directly without splitting strings.
    quarantine_reasons = {}
    if len(quarantine) > 0 and "failure_code" in quarantine.columns:
        quarantine_reasons = quarantine["failure_code"].value_counts().to_dict()
    ...
    # quarantine has one record per violation — use distinct tx_ids for the
    # "quarantined transactions" count so it stays comparable to input_rows.
    quarantine_tx_count = (
        quarantine["tx_id"].nunique()
        if len(quarantine) > 0 and "tx_id" in quarantine.columns
        else 0
    )
    ...
    return {
        ...
        "quarantine_rows":              quarantine_tx_count,
        "quarantine_violation_records": len(quarantine),
